...
> OAuth 2.0 authorization code + PKCE and device grant state machine for
> public clients, behind a host behaviour that mints the token.

[`managoat_oauth`](https://hex.pm/packages/managoat_oauth) on hex,
`Managoat.OAuth` in the code, Apache-2.0 throughout. It was extracted from
...
The grant state machine, client registry, config loader and migration are driven
through both an instance and the direct facade, against the library's own
Postgres database. The remaining misses are collision retries and
concurrency-loser paths, so there is a little headroom for a real branch.
...
**This is the one library whose suite needs Postgres.** `test/test_helper.exs`
drops, creates and migrates a database of its own on every run, using
`Managoat.OAuth.Migration` itself — so the schema under test is always the one
the code describes.

```bash
# the default; override with MANAGOAT_OAUTH_TEST_DATABASE_URL
postgres://postgres:postgres@localhost:5432/managoat_oauth_test
```

CI runs a `postgres:16` service for this and sets that variable; this is the
only library whose `ci.yml` carries the block.

`DATABASE_URL` is **deliberately not read**. On a developer machine that
variable names the host application's database, and this suite drops the
database it is pointed at.

`test/support/` carries `case.ex`, `instances.ex`, `recording_host.ex` and
`test_repo.ex`. The suite runs two instances against a repo of its own, never
the host's: under a host umbrella every app's tests share one VM, so a repo
pointed at the host's test database would be a second sandbox owner on the same
tables.

`.formatter.exs` here carries `import_deps: [:ecto, :ecto_sql]`, which the other
libraries do not.

### The async config guardrail

`async_global_config_guardrail_test.exs` fails the suite if any `async: true`
test module writes `Application.put_env(:managoat_oauth, ...)`.
...
- **Don't point the test repo at `DATABASE_URL`.** The suite drops the
  database it is given.

- **Don't loosen exact redirect-URI matching.** README.md has the section;
  prefix or wildcard matching on a redirect URI is an open redirector.
