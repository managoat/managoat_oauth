# Contributing to Managoat.OAuth
...
This suite needs Postgres. It drops, creates and migrates a database of its
own on every run; override the URL with `MANAGOAT_OAUTH_TEST_DATABASE_URL`.
`DATABASE_URL` is deliberately not read, because on a developer machine it
names the host application's database.
